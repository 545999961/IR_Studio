...
class LLMInstructLogitAgent():
    ...
    def generate(self,
        ...
        sampling_params = SamplingParams(temperature=temperature, top_p=top_p, max_tokens=max_tokens, 
                                         stop = stop, n=n,
                                         logprobs=1,
                                         use_beam_search=True, best_of=200,
        ...
        s = 0
        for k in outputs[0].outputs[0].logprobs:
            s += list(k.values())[0].logprob
                ...
                results.append(
                    [
                        (
                            ...
                            sum([list(k.values())[0].logprob for k in output.outputs[i].logprobs]) / len([list(k.values())[0].logprob for k in output.outputs[i].logprobs])
                        )
        ...
        return [
            (
                ...
                sum([list(k.values())[0].logprob for k in output.outputs[0].logprobs]) / len([list(k.values())[0].logprob for k in output.outputs[0].logprobs])
            ) for output in outputs
        ]
...
# a = LLMInstructLogitAgent('/share/chaofan/models/Llama-3.2-1B-Instruct')
# print(a.generate(['hello, nice to see you!'] * 2, n=20))