...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # full_encodeds = self.tokenizer.apply_chat_template(messages_full)
        # inputs_encodeds = self.tokenizer.apply_chat_template(messages)

        tokenized_full_prompt = self.tokenizer(inputs)
        tokenized_full_prompt['input_ids'] = full_encodeds.copy()
        tokenized_full_prompt['attention_mask'] = [1] * len(full_encodeds)
        tokenized_full_prompt['labels'] = full_encodeds.copy()
        tokenized_full_prompt['labels'] = [-100] * len(inputs_encodeds) + tokenized_full_prompt['input_ids'][len(inputs_encodeds):]
